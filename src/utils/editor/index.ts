...
import {
  parseHTMLToParagraph,
  parseParagraphToMarkdown,
} from './parser/paragraph'
import { parseHTMLToList, parseListToMarkdown } from './parser/list'
import { delimiter, parseDelimiterToMarkdown } from './parser/delimiter'
...
import { parseKatexToMarkdown } from '@/utils/editor/parser/katex'
...
export const parseMarkdownToEditorJs = (markdown: string) => {
  markdown = markdown
    ...
    .replaceAll(
      /\$\$([^$]+)\$\$/g,
      (_, mathExp: string) => `<katex>${mathExp.trim()}</katex>`
    )
    .replaceAll('\n\n', '\n')
    .replaceAll('\n', '\n\n')
